...
juce::Rectangle<float> EnvelopeDisplay::getPlotBounds() const
{
    ...
    const float topTrim = juce::jlimit(32.0f, 40.0f, bounds.getHeight() * 0.24f);
      ...
      lnf_(accent)
{
    ...
    headerComponent_->setAccent(accent_);
    if (headerLogoImage_.isValid())
        headerComponent_->setLogo(headerLogoImage_);
    addAndMakeVisible(*headerComponent_);

    // ---- Gain dial (kept as member for APVTS lifetime, delegated to Header) ----
    // Note: gainDial widget is still owned by CommonSynthEditor (APVTS requirement).
    // It is repositioned by HeaderComponent::resized() but we must call resized()
    // on headerComponent_ after we know our bounds.
    // ---- Single note toggle ----
...
CommonSynthEditor::HeaderZones CommonSynthEditor::computeHeaderZones(int headerH) const
{
    ...
    const bool compact = getWidth() < 1120 || getHeight() < 700;
    const int identityW = compact
        ? juce::jlimit(184, 216, static_cast<int>(std::round(contentW * 0.17f)))
        : juce::jlimit(196, 236, static_cast<int>(std::round(contentW * 0.18f)));
    const int statusW = compact
        ? juce::jlimit(260, 336, static_cast<int>(std::round(contentW * 0.26f)))
        : juce::jlimit(284, 376, static_cast<int>(std::round(contentW * 0.28f)));
...
// =============================================================================
// Chrome layout — positions headerComponent_ and keyboard
// =============================================================================
...
CommonSynthEditor::resizeChrome(int presetsStartX, int selectorH, int kbH)
{
    ...
    // ---- Header component (V5 refactored, self-positions its children) ----
    // Density: 0 = compact (<1120px or <700px), 0.5 = normal, 1 = roomy (>1600px or >940px)
    const bool compact = w < 1120 || getHeight() < 700;
    const bool roomy = w > 1600 || getHeight() > 940;
    const float density = compact ? 0.2f : (roomy ? 0.8f : 0.5f);
    headerComponent_->setDensity(density);
    headerComponent_->setBounds(margin, 8, w - margin * 2, headerH - 2);
    // HeaderComponent::resized() positions gainDial, presetSearch, presetBox,
    // prev/next buttons, and action buttons internally.
