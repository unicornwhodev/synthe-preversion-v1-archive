...
public:
    ...
    bool hasSignificantContent(float threshold) const
    {
...
struct OnePole
{
    ...
    bool  hasSignificantState(float threshold) const { return std::abs(state) > threshold; }
};
...
public:
    ...
    bool hasSignificantState(float threshold) const
    {
        return delay.hasSignificantContent(threshold);
    }
...
public:
    ...
    void process(float* left, float* right, int numSamples, const Params& p)
    {
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            const float lfo = mis::fastSin(modPhase);
            modPhase += modRate;
    ...
    bool hasAudibleTail(float threshold = 1.0e-4f) const
    {
        if (preDelay.hasSignificantContent(threshold))
            return true;

        for (const auto& diffuser : inDiff)
        {
            if (diffuser.hasSignificantState(threshold))
                return true;
        }

        for (const auto& apf : tankModApf)
        {
            if (apf.hasSignificantState(threshold))
                return true;
        }

        for (const auto& delay : tankDelay)
        {
            if (delay.hasSignificantContent(threshold))
                return true;
        }

        for (const auto& damp : tankDamp)
        {
            if (damp.hasSignificantState(threshold))
                return true;
        }

        for (const auto& bandwidth : inBandwidth)
        {
            if (bandwidth.hasSignificantState(threshold))
                return true;
        }

        return std::abs(tankState[0]) > threshold || std::abs(tankState[1]) > threshold;
    }
...
public:
    ...
    void process(float* left, float* right, int numSamples, const Params& p)
    {
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            for (int c = 0; c < numCh; ++c)
            {
                const float lfo = mis::fastSin(lfoPhase[c]);
    ...
    bool hasAudibleTail(float threshold = 1.0e-4f) const
    {
        for (const auto& line : delay)
        {
            if (line.hasSignificantContent(threshold))
                return true;
        }
        return false;
    }
