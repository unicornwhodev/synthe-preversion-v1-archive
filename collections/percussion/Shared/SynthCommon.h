...
protected:
    ...
    void paintHeader(juce::Graphics& g, int headerH, int contentX, int contentW) const;
    ...
    /// Pass contentX/contentW to align zones with actual content area origin.
    HeaderZones computeHeaderZones(int headerH, int contentX, int contentW) const;
    ...
    // Gain (APVTS bound to "output_gain")
    juce::Slider         gainDial;

    // Optional single-note toggle (add to parent if plugin uses it)
